/* Test ECI 29 GB 2312 Chinese (PRC) */
/*
...
static void test_utf8_to_eci(const testCtx *const p_ctx) {
    ...
    static const struct item data[] = {
        ...
        /*  9*/ { TU("\xE9\xBD\x84"), -1, 0, 2 }, /* U+9F44 last Han mapped */
        /* 10*/ { TU("\xE9\xBD\x85"), -1, ZUECI_ERROR_INVALID_DATA, -1 }, /* U+9F45 unmapped */
        /* 11*/ { TU("A\xC2\xA4\xE9\xBD\x84~\xE3\x80\x80\x7F\xC2\xA7"), -1, 0, 11 }, /* Multiple (good) mappings */
    };
...
#ifdef ZUECI_TEST_ICONV
    cd_from_u = iconv_open("GB2312" /*to*/, "UTF-8" /*from*/);
...
static void test_eci_to_utf8(const testCtx *const p_ctx) {
    ...
    static const struct item data[] = {
        ...
        /* 10*/ { TU("\xA0\x40"), -1, 0, 0, ZUECI_ERROR_INVALID_DATA, -1 }, /* 0xA0 invalid leading byte */
        /* 11*/ { TU("\xA0\x40"), -1, 0x00BF, 0, ZUECI_WARN_INVALID_DATA, 2 }, /* -> U+00BF */
        /* 12*/ { TU("\xA0\x40G"), -1, 0x00BF, 0, ZUECI_WARN_INVALID_DATA, 3 }, /* -> U+00BF G */
        /* 13*/ { TU("\x00"), 1, 0, 0, 0, 1 }, /* NUL */
        /* 14*/ { TU("A\xA1\xA7\xE1\xC0;\xA4\xF3\x7F"), -1, 0, 0, 0, 11 }, /* -> A U+00AE U+5D02 ; U+3093 <DEL> */
        /* 15*/ { TU("\xA4\xF4~\xA1\xA7\xE1\xC0;\xA4\xF3\xA4\xF5\x7F"), -1, 0x00BF, 0, ZUECI_WARN_INVALID_DATA, 15 }, /* -> U+00BF ~ U+00AE U+5D02 ; U+3093 U+00BF <DEL> */
    };
    int data_size = ZUECI_ASIZE(data);
    int i, src_len, ret;
    const int eci = 29;
...
/* As control convert to GB 2312 using simple table generated from
   unicode.org-mappings/EASTASIA/GB/GB2312.TXT plus simple processing
   (see https://haible.de/bruno/charsets/conversion-tables/GB2312.tar.bz2) */
static int u_gb2312_2(zueci_u32 u, unsigned char *dest) {
    ...
    tab_length = ZUECI_ASIZE(test_gb2312_tab);
    ...
    for (i = start_i; i < end_i; i += 2) {
        if (test_gb2312_tab[i + 1] == u) {
            mb = test_gb2312_tab[i] + 0x8080; /* Table in GB 2312 not EUC-CN */
            if (mb & 0xFF00) {
...
static void test_u_gamut(const testCtx *const p_ctx) {
    ...
    for (i = 0; i < 0xFFFE; i++) { /* Exclude not-a-characters reverse BOM 0xFFFE and 0xFFFF */
        ...
        ret = zueci_u_gb2312_test(i, val);
        ret2 = u_gb2312_2(i, val2);
        ...
        if (ret) {
            ...
            u3 = 0;
            ret3 = zueci_gb2312_u_test(val, ret, 0, &u3);
...
/* Test all GB 2312 codepoints */
static void test_gb2312_gamut(const testCtx *const p_ctx) {
    ...
    for (i = 0xA1; i < 0xF8; i++) {
        ...
        for (j = 0xA1; j < 0xFE; j++) {
            ...
            ret = zueci_gb2312_u_test(src, 2, 0, &u);
#ifndef ZUECI_EMBED_NO_TO_ECI
            if (ret) {
                unsigned char dest[4] = {0};
                int ret2 = zueci_u_gb2312_test(u, dest);
...
#endif
#ifndef ZUECI_EMBED_NO_TO_UTF8
        { "test_gb2312_gamut", test_gb2312_gamut },
#endif
    };
