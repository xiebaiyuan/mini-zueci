...
static void test_utf8_to_eci(const testCtx *const p_ctx) {
    ...
    static const struct item data[] = {
        ...
        /* 11*/ { TU("\xEF\xA8\xA9"), -1, 0, 2 }, /* U+FA29 last CJK mapping */
        /* 12*/ { TU("\xEF\xA8\xAA"), -1, ZUECI_ERROR_INVALID_DATA, -1 }, /* U+FA2A CJK unmapped */
        /* 13*/ { TU("A\xC2\xA4\xE9\xBD\x84\xE9\xBD\x85~\xE3\x80\x80\x7F\xC2\xA7\xEF\xA8\xA9"), -1, 0, 15 }, /* Multiple (good) mappings */
        /* 14*/ { TU("\xE2\x82\xAC"), -1, ZUECI_ERROR_INVALID_DATA, -1 }, /* U+20AC EURO SIGN unmapped (ignored although in Unicode Public/MAPPINGS/VENDORS/MICSFT/WINDOWS/CP936.TXT) */
    };
...
#ifdef ZUECI_TEST_ICONV
    cd_from_u = iconv_open("GBK" /*to*/, "UTF-8" /*from*/);
    assert_zero(cd_from_u == (iconv_t) -1, "iconv_open cd_from_u == -1\n");
    cd_to_u = iconv_open("UTF-8" /*to*/, "GBK" /*from*/);
    ...
    for (i = 0; i < data_size; i++) {
        ...
        if (ret == 0) {
            ...
            /* Check no iconv mapping */
            {
                ...
                assert_nonzero(from_u_ret, "i:%d unexpected iconv mapping\n", i);
            }
#endif
        }
    }
...
static void test_eci_to_utf8(const testCtx *const p_ctx) {
    ...
    static const struct item data[] = {
        ...
        /*  3*/ { TU("\xA4\xF4"), -1, 0, 0, ZUECI_ERROR_INVALID_DATA, -1 }, /* 0xA4F4 unmapped */
        /*  4*/ { TU("\xA4\xF4"), -1, '?', 0, ZUECI_WARN_INVALID_DATA, 1 }, /* -> ? */
        /*  5*/ { TU("\xA4\xF4"), -1, 0xFFFD, 0, ZUECI_WARN_INVALID_DATA, 3 }, /* -> U+FFFD */
        /*  6*/ { TU("\xA1\x7F"), -1, 0, 0, ZUECI_ERROR_INVALID_DATA, -1 }, /* 0xA17F invalid trailing byte */
        /*  7*/ { TU("\xA1\xFF"), -1, 0, 0, ZUECI_ERROR_INVALID_DATA, -1 }, /* 0xA1FF invalid trailing byte */
        /*  8*/ { TU("\xA1"), -1, 0, 0, ZUECI_ERROR_INVALID_DATA, -1 }, /* 0xA1 missing trailing byte */
        /*  9*/ { TU("\xA1"), -1, 0xFFFD, 0, ZUECI_WARN_INVALID_DATA, 3 }, /* -> U+FFFD */
        /* 10*/ { TU("\xA1\x3F"), -1, 0xFFFD, 0, ZUECI_WARN_INVALID_DATA, 3 }, /* -> U+FFFD (invalid trailing byte) */
        /* 11*/ { TU("\xA1\x3FG"), -1, 0xFFFD, 0, ZUECI_WARN_INVALID_DATA, 4 }, /* -> U+FFFD G */
        /* 12*/ { TU("\x80\x40"), -1, 0, 0, ZUECI_ERROR_INVALID_DATA, -1 }, /* 0x80 invalid leading byte */
        /* 13*/ { TU("\x80\x40"), -1, 0x00BF, 0, ZUECI_WARN_INVALID_DATA, 2 }, /* -> U+00BF */
        /* 14*/ { TU("\xFF\x40"), -1, 0, 0, ZUECI_ERROR_INVALID_DATA, -1 }, /* 0xFF invalid leading byte */
        /* 15*/ { TU("\xFF\x40"), -1, 0x00BF, 0, ZUECI_WARN_INVALID_DATA, 2 }, /* -> U+00BF */
        /* 16*/ { TU("\xFF\x40G"), -1, 0x00BF, 0, ZUECI_WARN_INVALID_DATA, 3 }, /* -> U+00BF G */
        /* 17*/ { TU("\x00"), 1, 0, 0, 0, 1 }, /* NUL */
        /* 18*/ { TU("A\xA1\xA7\xE1\xC0;\xA4\xF3\x7F"), -1, 0, 0, 0, 11 }, /* -> A U+00AE U+5D02 ; U+3093 <DEL> */
        /* 19*/ { TU("\xA4\xF4~\xA1\xA7\xE1\xC0;\xA4\xF3\xA4\xF5\x7F"), -1, '?', 0, ZUECI_WARN_INVALID_DATA, 13 }, /* -> ? ~ U+00AE U+5D02 ; U+3093 ? <DEL> */
        /* 20*/ { TU("\x80"), -1, 0, 0, ZUECI_ERROR_INVALID_DATA, -1 }, /* 0x80 mapped to U+20AC (EURO SIGN) in Unicode Public mapping file (ignored) */
    };
    int data_size = ZUECI_ASIZE(data);
    int i, src_len, ret;
    const int eci = 31;
...
#ifdef ZUECI_TEST_ICONV
    cd_to_u = iconv_open("UTF-8" /*to*/, "GB2312" /*from*/);
    assert_zero(cd_to_u == (iconv_t) -1, "iconv_open cd_to_u == -1\n");
#endif
    ...
    for (i = 0; i < data_size; i++) {
        ...
        if (ret < ZUECI_ERROR) {
            ...
            /* Check no iconv mapping */
            {
                ...
                assert_nonzero(to_u_ret, "i:%d unexpected iconv mapping\n", i);
            }
#endif
        }
    }
...
/* As control convert to GBK using simple table generated from
   https://unicode.org/Public/MAPPINGS/VENDORS/MICSFT/WINDOWS/CP936.TXT plus simple processing */
static int u_gbk_2(zueci_u32 u, unsigned char *dest) {
    int tab_length, start_i, end_i;
    int i;
    zueci_u32 mb;

    tab_length = ZUECI_ASIZE(test_gbk_tab);
...
static void test_u_gamut(const testCtx *const p_ctx) {
    ...
    for (i = 0; i < 0xFFFE; i++) { /* Exclude not-a-characters reverse BOM 0xFFFE and 0xFFFF */
        ...
        if (ret) {
            ...
            u3 = 0;
            ret3 = zueci_gbk_u_test(val, ret, 0, &u3);
...
/* Test all GBK codepoints */
static void test_gbk_gamut(const testCtx *const p_ctx) {
    ...
    for (i = 0x81; i < 0xFF; i++) {
        ...
        for (j = 0x40; j < 0xFF; j++) {
            ...
            ret = zueci_gbk_u_test(src, 2, 0, &u);
#ifndef ZUECI_EMBED_NO_TO_ECI
            if (ret) {
                unsigned char dest[4] = {0};
                int ret2 = zueci_u_gbk_test(u, dest);
