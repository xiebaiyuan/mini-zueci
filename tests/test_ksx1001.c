/* Test ECI 30 EUC-KR (KS X 1001, formerly KS C 5601) Korean */
/*
...
static void test_utf8_to_eci(const testCtx *const p_ctx) {
    ...
    static const struct item data[] = {
        ...
        /*  9*/ { TU("\xE9\xBE\x9C"), -1, 0, 2 }, /* U+9F9C last Han mapped */
        /* 10*/ { TU("\xE9\xBE\x9D"), -1, ZUECI_ERROR_INVALID_DATA, -1 }, /* U+9F9D unmapped */
        /* 11*/ { TU("\xEF\xBF\xA6"), -1, 0, 2 }, /* U+FFE6 FULLWIDTH WON SIGN */
        /* 12*/ { TU("\xEF\xBF\xA7"), -1, ZUECI_ERROR_INVALID_DATA, -1 }, /* U+FFE7 unmapped */
        /* 13*/ { TU("A\xEF\xBF\xA6\xC2\xA4\x7F\xE9\xBE\x9C~"), -1, 0, 9 }, /* Multiple (good) mappings */
    };
...
#ifdef ZUECI_TEST_ICONV
    cd_from_u = iconv_open("EUC-KR" /*to*/, "UTF-8" /*from*/);
    assert_zero(cd_from_u == (iconv_t) -1, "iconv_open cd_from_u == -1\n");
    cd_to_u = iconv_open("UTF-8" /*to*/, "EUC-KR" /*from*/);
...
static void test_eci_to_utf8(const testCtx *const p_ctx) {
    ...
    /* s/\/\*[ 0-9]*\*\//\=printf("\/\*%3d*\/", line(".") - line("'<")): */
    static const struct item data[] = {
        /*  0*/ { TU("\xA2\xE7"), -1, 0, 0, 0, 2 }, /* 0xA2E7 -> U+00AE */
        /*  1*/ { TU("\xA2\xE8"), -1, 0, 0, 0, 3 }, /* 0xA2E8 -> U+327E */
        /*  2*/ { TU("\xA2\xE9"), -1, 0, 0, ZUECI_ERROR_INVALID_DATA, -1 }, /* 0xA2E9 unmapped */
        /*  3*/ { TU("\xA2\xE9"), -1, '?', 0, ZUECI_WARN_INVALID_DATA, 1 }, /* -> ? */
        /*  4*/ { TU("\xA2\xE9"), -1, 0xFFFD, 0, ZUECI_WARN_INVALID_DATA, 3 }, /* -> U+FFFD */
        /*  5*/ { TU("\xA1\x7F"), -1, 0, 0, ZUECI_ERROR_INVALID_DATA, -1 }, /* 0xA17F invalid trailing byte */
        ...
        /*  9*/ { TU("\xA1\x3F"), -1, 0xFFFD, 0, ZUECI_WARN_INVALID_DATA, 3 }, /* -> U+FFFD (invalid trailing byte) */
        /* 10*/ { TU("\xA1\x3FG"), -1, 0xFFFD, 0, ZUECI_WARN_INVALID_DATA, 4 }, /* -> U+FFFD G */
        /* 11*/ { TU("\xA0\x40"), -1, 0, 0, ZUECI_ERROR_INVALID_DATA, -1 }, /* 0x80 invalid leading byte */
        /* 12*/ { TU("\xA0\x40"), -1, 0x00BF, 0, ZUECI_WARN_INVALID_DATA, 2 }, /* -> U+00BF */
        /* 13*/ { TU("\xA0\x40G"), -1, 0x00BF, 0, ZUECI_WARN_INVALID_DATA, 3 }, /* -> U+00BF G */
        /* 14*/ { TU("\x00"), 1, 0, 0, 0, 1 }, /* NUL */
        /* 15*/ { TU("A\xA2\xE7\xA2\xE8~B\xFD\xFE" "C"), -1, 0, 0, 0, 12 }, /* -> A U+00AE U+327E ~ B U+8A70 C */
        /* 16*/ { TU("A\xA2\xE9\xA2\xE7\xA1\x7F\xA2\xE8~B\xFD\xFE" "C"), -1, 0xFFFD, 0, ZUECI_WARN_INVALID_DATA, 18 }, /* -> A U+FFFD U+00AE U+FFFD U+327E ~ B U+8A70 C */
    };
    int data_size = ZUECI_ASIZE(data);
    int i, src_len, ret;
    const int eci = 30;
...
/* As control convert to ksx1001 using simple table generated from
   https://unicode.org/Public/MAPPINGS/OBSOLETE/EASTASIA/KSC/KSX1001.TXT plus simple processing */
static int u_ksx1001_2(zueci_u32 u, unsigned char *dest) {
    ...
    if (u == 0x20AC) { /* Euro sign added KS X 1001:1998 */
        mb = 0x2266 + 0x8080;
    } else if (u == 0xAE) { /* Registered trademark added KS X 1001:1998 */
        mb = 0x2267 + 0x8080;
    } else if (u == 0x327E) { /* Korean postal code symbol added KS X 1001:2002 */
        mb = 0x2268 + 0x8080;
    }
    if (mb) {
    ...
    tab_length = ZUECI_ASIZE(test_ksx1001_tab);
...
static void test_u_gamut(const testCtx *const p_ctx) {
    ...
    for (i = 0; i < 0xFFFE; i++) { /* Exclude not-a-characters reverse BOM 0xFFFE and 0xFFFF */
        ...
        ret = zueci_u_ksx1001_test(i, val);
        ret2 = u_ksx1001_2(i, val2);
        ...
        if (ret) {
            ...
            u3 = 0;
            ret3 = zueci_ksx1001_u_test(val, ret, 0, &u3);
...
/* Test all EUC-KR (KS X 1001) codepoints */
static void test_ksx1001_gamut(const testCtx *const p_ctx) {
    ...
    for (i = 0xA1; i < 0xFE; i++) {
        ...
        for (j = 0xA1; j < 0xFE; j++) {
            ...
            ret = zueci_ksx1001_u_test(src, 2, 0, &u);
#ifndef ZUECI_EMBED_NO_TO_ECI
            if (ret) {
                unsigned char dest[4] = {0};
                int ret2 = zueci_u_ksx1001_test(u, dest);
...
#endif
#ifndef ZUECI_EMBED_NO_TO_UTF8
        { "test_ksx1001_gamut", test_ksx1001_gamut },
#endif
    };
