...
static void test_utf8_to_eci(const testCtx *const p_ctx) {
    ...
    static const struct item data[] = {
        ...
        /*  1*/ { TU(" !\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}\x7F"), 95, 0, 95 + 1 }, /* Backslash goes to 2 byte */
        ...
        /*  8*/ { TU("~"), -1, ZUECI_ERROR_INVALID_DATA, -1 }, /* No mapping for tilde */
        /*  9*/ { TU("\xC2\xA1"), -1, ZUECI_ERROR_INVALID_DATA, -1 }, /* No mapping for U+00A1 Inverted exclamation mark */
        /* 10*/ { TU("\xC2\xA5"), -1, 0, 1 }, /* U+00A5 Yen goes to backslash */
        /* 11*/ { TU("\xC2\xBF"), -1, ZUECI_ERROR_INVALID_DATA, -1 }, /* No mapping for U+00BF Inverted question mark */
        /* 12*/ { TU("\xC3\x80"), -1, ZUECI_ERROR_INVALID_DATA, -1 }, /* No mapping for U+00C0 À */
        /* 13*/ { TU("\xC3\xA9"), -1, ZUECI_ERROR_INVALID_DATA, -1 }, /* No mapping for U+00E9 é */
        /* 14*/ { TU("\xCA\xB2"), -1, ZUECI_ERROR_INVALID_DATA, -1 }, /* No mapping for U+03B2 β */
        /* 15*/ { TU("\xE2\x80\xBE"), -1, 0, 1 }, /* U+203E OVERLINE goes to tilde */
        /* 16*/ { TU("\xE2\xBA\x80"), -1, ZUECI_ERROR_INVALID_DATA, -1 }, /* No mapping for U+2E80 CJK RADICAL REPEAT */
        /* 17*/ { TU("\xE3\x80\x84"), -1, ZUECI_ERROR_INVALID_DATA, -1 }, /* No mapping for U+3004 JAPANESE INDUSTRIAL STANDARD SYMBOL */
        /* 18*/ { TU("\xE3\x81\x81"), -1, 0, 2 }, /* U+3041 HIRAGANA LETTER SMALL A */
        /* 19*/ { TU("\xE9\xBE\xA0"), -1, 0, 2 }, /* U+9FA0 last URO mapping */
        /* 20*/ { TU("\xE9\xBE\xA1"), -1, ZUECI_ERROR_INVALID_DATA, -1 }, /* U+9FA1 No mapping (URO) */
        /* 21*/ { TU("\xEE\x80\x80"), -1, 0, 2 }, /* U+E000 PUA */
        /* 22*/ { TU("\xEE\x9D\x97"), -1, 0, 2 }, /* U+E757 PUA */
        /* 23*/ { TU("\xEE\x9D\x98"), -1, ZUECI_ERROR_INVALID_DATA, -1 }, /* U+E758 No mapping (PUA) */
        /* 24*/ { TU("\xEF\xBC\x81"), -1, 0, 2 }, /* U+FF01 FULLWIDTH EXCLAMATION MARK 1st mapping after PUA */
        /* 25*/ { TU("A\xC2\xA5\xE2\x80\xBE\xE3\x80\x80\xE3\x81\x81G"), -1, 0, 8 }, /* Multiple (good) mappings */
    };
...
#ifdef ZUECI_TEST_ICONV
    cd_from_u = iconv_open("SHIFT_JIS" /*to*/, "UTF-8" /*from*/);
    assert_zero(cd_from_u == (iconv_t) -1, "iconv_open cd_from_u == -1\n");
    cd_to_u = iconv_open("UTF-8" /*to*/, "SHIFT_JIS" /*from*/);
...
#ifdef ZUECI_TEST_ICONV
            if (strchr((const char *) data[i].data, '\\') == NULL) { /* iconv does not map backslash to 0x815F */
...
static void test_eci_to_utf8(const testCtx *const p_ctx) {
    ...
    /* s/\/\*[ 0-9]*\*\//\=printf("\/\*%3d*\/", line(".") - line("'<")): */
    static const struct item data[] = {
        /*  0*/ { TU("~"), -1, 0, 0, 0, 3 }, /* -> U+203E */
        /*  1*/ { TU("~"), -1, 0, ZUECI_FLAG_SJIS_STRAIGHT_THRU, 0, 1 }, /* -> U+007E */
        /*  2*/ { TU("\\"), -1, 0, 0, 0, 2 }, /* -> U+00A5 */
        /*  3*/ { TU("\\"), -1, 0, ZUECI_FLAG_SJIS_STRAIGHT_THRU, 0, 1 }, /* -> U+005C */
        /*  4*/ { TU("~\xEA\xA4"), -1, 0, 0, 0, 6 }, /* -> U+203E U+7199 */
        /*  5*/ { TU("~\xEA\xA4"), -1, 0, ZUECI_FLAG_SJIS_STRAIGHT_THRU, 0, 4 }, /* -> U+00A5 U+7199 */
        /*  6*/ { TU("~\xEA\xA5"), -1, 0, 0, ZUECI_ERROR_INVALID_DATA, -1 }, /* 0xEAA5 invalid */
        /*  7*/ { TU("~\xEA\xA5"), -1, 0, ZUECI_FLAG_SJIS_STRAIGHT_THRU, ZUECI_ERROR_INVALID_DATA, -1 },
        /*  8*/ { TU("~\xEA\xA5"), -1, '?', 0, ZUECI_WARN_INVALID_DATA, 4 }, /* -> U+203E ? */
        /*  9*/ { TU("~\xEA\xA5"), -1, 0xFFFD, 0, ZUECI_WARN_INVALID_DATA, 6 }, /* -> U+203E U+FFFD */
        /* 10*/ { TU("~\xEA\xA5"), -1, 0xFFFD, ZUECI_FLAG_SJIS_STRAIGHT_THRU, ZUECI_WARN_INVALID_DATA, 4 }, /* -> U+007E U+FFFD */
        /* 11*/ { TU("~\xEA\xA5\xEA\xA4"), -1, '?', 0, ZUECI_WARN_INVALID_DATA, 7 }, /* -> U+203E U+FFFD U+7199 */
        /* 12*/ { TU("\x81\x3F"), -1, 0, 0, ZUECI_ERROR_INVALID_DATA, -1 }, /* 0x813F invalid (min Shift JIS 0x8140) */
        /* 13*/ { TU("\x81\x3F"), -1, 0x00BF, 0, ZUECI_WARN_INVALID_DATA, 2 }, /* -> U+00BF */
        /* 14*/ { TU("\x81\x3FG"), -1, 0x00BF, 0, ZUECI_WARN_INVALID_DATA, 3 }, /* -> U+00BF G */
        /* 15*/ { TU("\xF9\xFC"), -1, 0, 0, 0, 3 }, /* 0xF9FC PUA max -> U+E757 */
        /* 16*/ { TU("\x81\x7F"), -1, 0, 0, ZUECI_ERROR_INVALID_DATA, -1 }, /* 0x817F invalid trailing byte */
        /* 17*/ { TU("\x81\xFD"), -1, 0, 0, ZUECI_ERROR_INVALID_DATA, -1 }, /* 0x81FD invalid trailing byte */
        /* 18*/ { TU("\x81"), -1, 0, 0, ZUECI_ERROR_INVALID_DATA, -1 }, /* 0x81 missing trailing byte */
        /* 19*/ { TU("\xA1"), -1, 0, 0, 0, 3 }, /* Half-width katakana U+FF61 */
        /* 20*/ { TU("\xA1"), -1, 0, ZUECI_FLAG_SJIS_STRAIGHT_THRU, 0, 3 }, /* -> U+FF61 (straight-thru ASCII only) */
        /* 21*/ { TU("\x80\x40"), -1, 0, 0, ZUECI_ERROR_INVALID_DATA, -1 }, /* 0x80 bad lead byte */
        /* 22*/ { TU("\x80\x40"), -1, 0xFFFD, 0, ZUECI_WARN_INVALID_DATA, 3 }, /* -> U+FFFD */
        /* 23*/ { TU("\x80\x40G"), -1, 0xFFFD, 0, ZUECI_WARN_INVALID_DATA, 4 }, /* -> U+FFFD G */
        /* 24*/ { TU("\xFA\x40"), -1, 0, 0, ZUECI_ERROR_INVALID_DATA, -1 }, /* 0xFA bad lead byte */
        /* 25*/ { TU("\x81\xAD"), -1, 0, 0, ZUECI_ERROR_INVALID_DATA, -1 }, /* 0x81AD unmapped */
        /* 26*/ { TU("A\xF9\xFC\xEA\xA4\xA1\x7F\\\x81\xACG"), -1, 0, 0, 0, 17 }, /* -> A U+F9FC U+7199 U+FF61 <DEL> \ U+3013 G */
        /* 27*/ { TU("A\x81\xAD\xF9\xFC\x81\x7F\xEA\xA4\xA1\x7F\\\x81\xAC~G"), -1, 0xFFFD, 0, ZUECI_WARN_INVALID_DATA, 26 }, /* -> A U+FFFD U+E757 U+FFFD U+7199 U+FF61 <DEL> U+00A5 U+3013 U+202E G */
    };
    int data_size = ZUECI_ASIZE(data);
    int i, src_len, ret;
    const int eci = 20;
    ...
    for (i = 0; i < data_size; i++) {
        ...
        if (ret < ZUECI_ERROR) {
            ...
            if (ret == 0 && (data[i].flags & ZUECI_FLAG_SJIS_STRAIGHT_THRU) == 0) {
#ifdef ZUECI_TEST_ICONV
                if (strchr((const char *) data[i].data, '\\') == NULL) { /* iconv does not map backslash to 0x815F */
                    char out_to_u_buf[1024] = {0};
...
static int u_sjis_2(const zueci_u32 u, unsigned char *dest) {
    ...
    if (u >= 0xE000 && u <= 0xE757) { /* PUA mappings, not in SHIFTJIS.TXT */
        if (u <= 0xE0BB) {
            mb = u - 0xE000 + 0xF040 + (u >= 0xE000 + 0x3F);
        } else if (u <= 0xE177) {
            mb = u - 0xE0BC + 0xF140 + (u >= 0xE0BC + 0x3F);
        } else if (u <= 0xE233) {
            mb = u - 0xE178 + 0xF240 + (u >= 0xE178 + 0x3F);
        } else if (u <= 0xE2EF) {
            mb = u - 0xE234 + 0xF340 + (u >= 0xE234 + 0x3F);
        } else if (u <= 0xE3AB) {
            mb = u - 0xE2F0 + 0xF440 + (u >= 0xE2F0 + 0x3F);
        } else if (u <= 0xE467) {
            mb = u - 0xE3AC + 0xF540 + (u >= 0xE3AC + 0x3F);
        } else if (u <= 0xE523) {
            mb = u - 0xE468 + 0xF640 + (u >= 0xE468 + 0x3F);
        } else if (u <= 0xE5DF) {
            mb = u - 0xE524 + 0xF740 + (u >= 0xE524 + 0x3F);
        } else if (u <= 0xE69B) {
            mb = u - 0xE5E0 + 0xF840 + (u >= 0xE5E0 + 0x3F);
        } else {
            mb = u - 0xE69C + 0xF940 + (u >= 0xE69C + 0x3F);
        }
        dest[0] = (unsigned char) (mb >> 8);
        dest[1] = (unsigned char) mb;
        return 2;
    }
    tab_length = ZUECI_ASIZE(test_sjis_tab);
...
static void test_u_gamut(const testCtx *const p_ctx) {
    ...
    for (i = 0; i < 0xFFFE; i++) { /* Exclude not-a-characters reverse BOM 0xFFFE and 0xFFFF */
        ...
        ret = zueci_u_sjis_test(i, val);
        ret2 = u_sjis_2(i, val2);
        ...
        if (ret) {
            ...
            u3 = 0;
            ret3 = zueci_sjis_u_test(val, ret, 0, &u3);
            assert_equal(ret3, ret2, "i:%d 0x%04X val 0x%02X%02X, ret3 %d != ret2 %d\n", (int) i, i, val[0], val[1], ret3, ret2);
            assert_equal(i, u3, "i:%d 0x%04X != u3 0x%04X, val 0x%02X%02X\n", (int) i, i, u3, val[0], val[1]);
#else
            (void)ret3; (void)u3;
#endif
        }
...
#ifdef ZUECI_TEST_ICONV
        if (i != '\\') { /* iconv does not map backslash to 0x815F */
            ...
            if (ret) {
            ...
            } else if (i != 0xFF3C) { /* iconv maps U+FF3C FULLWIDTH REVERSE SOLIDUS to 0x815F instead of backslash */
                size_t from_u_ret = 0;
                while (in_from_u_bytes_left) {
                ...
                assert_nonzero(from_u_ret, "i:%d 0x%04X from_u_ret == 0, out_from_u_buf %02X %02X %02X\n",
                    i, i, out_from_u_buf[0] & 0xFF, out_from_u_buf[1] & 0xFF, out_from_u_buf[2] & 0xFF);
            }
        }
#endif
    }
...
/* Test all Shift JIS codepoints */
static void test_sjis_gamut(const testCtx *const p_ctx) {
    ...
    for (i = 0x81; i < 0xF9; i++) {
        ...
        for (j = 0x40; j < 0xFC; j++) {
            ...
            ret = zueci_sjis_u_test(src, 2, 0, &u);
#ifndef ZUECI_EMBED_NO_TO_ECI
            if (ret) {
                unsigned char dest[4] = {0};
                int ret2 = zueci_u_sjis_test(u, dest);
...
#ifdef ZUECI_TEST_ICONV
            if (u != 0x5C) { /* iconv doesn't map backslash */
                ...
                if (ret) {
                    ...
                    assert_zero(in_to_u_bytes_left, "i:%d j:%d 0x%02X%02X, u 0x%04X, in_to_u_bytes_left %d != 0\n",
...
#endif
#ifndef ZUECI_EMBED_NO_TO_UTF8
        { "test_sjis_gamut", test_sjis_gamut },
#endif
    };
